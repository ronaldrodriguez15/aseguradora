...
class LocationController extends Controller
{
    ...
    public function generateReport(Request $request)
    {
        ...
        // Si es 'all' o está vacío, reporte de todos
        if (in_array('all', $usuarios) || empty($usuarios)) {
            $users = User::where('status', 1)->get();
        ...
        elseif (count($usuarios) === 1) {
            $user = User::findOrFail($usuarios[0]);
        ...
        // Si son múltiples usuarios específicos
        else {
            $users = User::whereIn('id', $usuarios)->where('status', 1)->get();

            $locations = UserLocation::whereIn('user_id', $usuarios)
                ->when($fechaDesde && $fechaHasta, function ($query) use ($fechaDesde, $fechaHasta) {
                    $query->whereBetween('fecha', [$fechaDesde, $fechaHasta]);
                })
                ->orderBy('fecha')
            ...
            $pdf = Pdf::loadView('reports.all_geolocation', compact('users', 'locations'));
            return $pdf->stream("Reporte_Usuarios_Seleccionados.pdf");
        }
    }
