    ...
    <div class="container">
        <form autocomplete="off" action="{{ route('usuarios.update', $usuario['id']) }}" method="post">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-6">
                    <label for="emailConfirm">Confirmación correo electrónico</label>
            ...
            <div class="form-row">
                <div class="form-group col-md-6">
                    ...
                    @php
                        ...
                        $userRoleId = optional($userRole)->id;
                        $userRoleName = optional($userRole)->name;
                    @endphp
                ...
                <div id="tipo_fondo_container" class="col-md-6 mb-3"
                    style="margin-top: 32px; display: {{ strtolower($userRoleName ?? '') === 'ventas' ? 'block' : 'none' }};">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <input type="checkbox" class="js-switch" name="tipo_fondo" id="tipo_fondo"
                            {{ $usuario->tipo_fondo == '1' ? 'checked' : '' }}>
                        <label class="form-check-label" for="tipo_fondo">
                            ¿Es tipo fondo?
                        </label>
                    </div>
                </div>
...
@section('js')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const tipoFondoContainer = document.getElementById('tipo_fondo_container');
            const tipoFondoInput = document.getElementById('tipo_fondo');
            ...
            function toggleEmpresas() {
                ...
                if (tipoFondoContainer) {
                    tipoFondoContainer.style.display = isVentas() ? 'block' : 'none';
                }

                if (!isVentas() && tipoFondoInput) {
                    if (tipoFondoInput.checked) {
                        if (tipoFondoInput.switchery) {
                            tipoFondoInput.click();
                        } else {
                            tipoFondoInput.checked = false;
                        }
                    }
                }
            }
